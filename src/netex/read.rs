...
pub fn read_netex_file<P: AsRef<path::Path>>(
    ...
    root.get_child("dataObjects", context.namespace.as_str()).unwrap().children()
        ...
        .map(|frame| 
            read_composite_data_frame(collections, &mut context, frame)
            .map_err(|_|
                format_err!(
                    "Reading Frame id={:?}", frame.attr("id").unwrap_or("undefined")
                )                
            )
        )
        .collect()
...
fn read_composite_data_frame(
    ...
    composite_frame.get_child("frames", &mut context.namespace).unwrap().children()
...
fn read_organisations(
    ...
    let companies : Vec<_> = organisations.children().map(
        |node| objects::Company {
    ...
    if companies.len() > 0 {
        ...
        let mut companies: Vec<objects::Company> = companies.into_iter().filter(|c| collections.companies.get_idx(&c.id).is_none()).collect();
        collections.companies.append(&mut companies);
    } else {
        context.first_operator_id = "default_company".to_string();
        if collections.companies.get_idx(&context.first_operator_id).is_none() {
...
fn read_site_frame(
    ...
    site_frame.get_child("stopPlaces", &context.namespace).unwrap().children()
        .map(|node| read_stop_place(
                collections,
                context,
                &node,
            )
...
fn read_stop_place_name(
    context: &mut NetexContext,
    stop_place: &Element,
) -> Option<String> {
    // This function aims to be usable from a StopPlace node or a Quay node.
    let direct_name = stop_place.get_child("Name", &context.namespace)
        .map(|s| s.text().to_string());
    let label = stop_place.get_child("Label", &context.namespace)
        .map(|s| s.text().to_string());
    let topographic_place_view_name = stop_place.get_child("TopographicPlaceView", &context.namespace)
        .map(|c| 
...
fn read_stop_place_coord(
    ...
    Coord {
        lon: location_node
                .map(|c| c.get_child("Longitude", &context.namespace)
                    .map_or(
                        0.0,
                        |s| s.text().parse().unwrap_or(0.0)
                    )
                ).unwrap_or(0.0),
        lat: location_node
                .map(|c| c.get_child("Latitude", &context.namespace)
                    .map_or(
                        0.0,
                        |s| s.text().parse().unwrap_or(0.0)
                    )
