...
| stop_access         | no       | pathways.txt                    |                     | For stop_points (location_type=0) only: Set to `1` (true) if the stop has no pathway connection to any stop_location (entrance/exit). Empty for stop_points connected to stop_locations via pathways, and for all other location types.                                                                             |
